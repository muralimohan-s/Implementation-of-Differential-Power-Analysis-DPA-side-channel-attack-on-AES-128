...
KeyExpansion (unsigned char *inputKey, unsigned char *expandedKeys) 
{
  
//The firs 16 bytes are the original key
    for (int i = 0; i < 16; i++)
...
tmp[12] =
    (unsigned char) (mul2[state[12]] ^ mul3[state[13]] ^ state[14] ^
		     state[15]);
  
tmp[13] =
    (unsigned char) (state[12] ^ mul2[state[13]] ^ mul3[state[14]] ^
		     state[15]);
  
tmp[14] =
    (unsigned char) (state[12] ^ state[13] ^ mul2[state[14]] ^
		     mul3[state[15]]);
...
AES_Encrypt (unsigned char *message, unsigned char *key) 
{
...
unsigned char message[] = "Hello worldb ;
